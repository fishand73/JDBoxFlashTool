...
| Telnet 失败恢复 | 开启 | 使用内置、固定哈希的 JDCOS `4.3.0.r4211`，写入前仍需单独确认 |
...
6. 保持“Telnet 开启失败时，使用内置原厂 r4211 恢复后重试”勾选。若不接受自动准备降级，可以取消勾选。
7. 建议勾选“显示详细日志”。不要随意勾选“强制继续”；只有人工核对设备型号和分区布局后才可使用，而且它不能绕过刷写校验。
8. 点击“开始执行”，等待日志显示完成。

### Telnet 开启失败时会发生什么

工具会先尝试 `r4546 set_port_forward` 和旧版 `set_iptv_info` 两种方法。只有管理员密码登录成功、
两种方法都失败时，才会准备内置的原厂 `4.3.0.r4211`：

1. 本地核对固定文件大小、MD5、SHA256 和 FIT 格式；
2. 通过只读接口确认路由器型号是雅典娜 `RE-CS-02`；
3. 把固件上传到路由器 `/tmp`，上传阶段可点击“取消当前任务”，不会写入闪存；
4. 核对路由器返回的文件大小、MD5 和 `firmware_check` 状态；
5. 显示红色高风险确认窗口；只有完整输入 `DOWNGRADE-R4211-...` 才会提交；
6. 通过原厂接口开始升级，等待路由器重启和管理登录恢复，然后再次开启 Telnet 并继续备份。

`r4211` 是全量固件，包含官方 U-Boot、启动链和系统，可能覆盖第三方 U-Boot，也可能清除设置。
提交写入后不能取消，3–5 分钟内不能断电。如果管理地址或密码被恢复，工具会停止并提示重新完成
首次配置，不会重复提交升级。
...
> “取消当前任务”可用于等待 U-Boot 启动、等待 Web 就绪和原厂 r4211 上传到 `/tmp`。进入实际
> 写入阶段后按钮会自动禁用。如果取消前已经收到 `UBOOT:ABORTED`，路由器可能仍停留在 U-Boot，
> 请手动访问管理地址或重启路由器。
...
| Telnet 未开启 | 保持管理页面可访问并查看两个策略的日志；已启用备用恢复时，按提示确认 r4211，重启后工具会自动重试 |
| r4211 后无法重新登录 | 固件可能清除了设置；不要重复升级，访问 `192.168.68.1` 完成首次配置后重新运行工具 |
